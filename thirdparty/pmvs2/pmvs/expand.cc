...
int Cexpand::expandSub(const Ppatch& orgppatch, const int id,
  ...
  //-----------------------------------------------------------------
  // NGHIA: Might be possible to use this as the entry point for the GPU
  // Add to GPU queue
  // Exit function
  // Wait for GPU results
  // Call expandSub2() to complete

  m_fm.m_optim.refinePatch(patch, id, 100);

  // Every below here should be in a separate function, expandSub2(...)
