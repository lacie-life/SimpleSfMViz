...
/*
  ...
  Cpatch cpy = patch; // refinePatchLM modifies patch, so keep a copy in case it fails

  if(!refinePatchLM(patch, id, 1000, 1)) {
    patch = cpy;
	refinePatchBFGS(patch, id, 1000, 1);
  }
*/
...
bool Coptim::refinePatchLM(Cpatch& patch, const int id,
                             const int time, const int ncc) {
  int idtmp = id;
  ...
  control.epsilon = 1e-3; // default step size is too small for floats
  control.maxcall = time;
...
int Coptim::grabTex(const Vec4f& coord, const Vec4f& pxaxis, const Vec4f& pyaxis,
  ...
  if (weight < m_fm.m_cosangleThreshold1)
    return 1;
