...
void CphotoSetS::getPAxes(const int index, const pmvsVec4f& coord, const pmvsVec4f& normal,
                          pmvsVec4f& pxaxis, pmvsVec4f& pyaxis) const{
  m_photos[index].getPAxes(coord, normal, pxaxis, pyaxis);
}

void CphotoSetS::grabTex(const int index, const int level, const pmvsVec2f& icoord,
                         const pmvsVec2f& xaxis, const pmvsVec2f& yaxis,
                         std::vector<pmvsVec3f>& tex, const int normalizef) const{
  m_photos[index].grabTex(level, icoord, xaxis, yaxis, m_size, tex, normalizef);
}

// grabTex given 3D sampling information
void CphotoSetS::grabTex(const int index, const int level, const pmvsVec4f& coord,
...
float CphotoSetS::incc(const std::vector<std::vector<pmvsVec3f> >& texs,
...
void CphotoSetS::getMinMaxAngles(const pmvsVec4f& coord, const std::vector<int>& indexes,
                                 float& minAngle, float& maxAngle) const {
  minAngle = M_PI;
  maxAngle = 0.0f;
  vector<pmvsVec4f> rays;  rays.resize((int)indexes.size());
...
int CphotoSetS::checkAngles(const pmvsVec4f& coord,
...
float CphotoSetS::computeDepth(const int index, const pmvsVec4f& coord) const {
...
void CphotoSetS::setDistances(void) {
  ...
  for (int i = 0; i < m_num; ++i) {
    pmvsVec4f ray0 = m_photos[i].m_oaxis;
    ray0[3] = 0.0f;
    for (int j = 0; j < m_num; ++j) {
      pmvsVec4f ray1 = m_photos[j].m_oaxis;
      ray1[3] = 0.0f;
